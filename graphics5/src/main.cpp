...
int main(int argc, char const *argv[])
{
    ...
    Cube cubes;
    Plain plain;
    Car car(glm::vec3(0,-5,0), glm::vec3(0.75, 0.75, 0.75), 180.0f);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        model = glm::mat4 (1.0f);
        model = glm::translate(model, floorPositions[2]);
        model = glm::scale(model, floorScale[1]);
        ...
        car.renderCar(objectShader, steerLeft, steerRight);
        car.setDrivingCar(driveCar);
